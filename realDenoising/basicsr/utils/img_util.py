...
def safe_astype(img_np, out_type):
    """安全的类型转换，处理无效值"""
    # 检查并处理 NaN 和无穷大值
    if not np.isfinite(img_np).all():
        # 使用 nan_to_num 替换 NaN 和无穷大
        img_np = np.nan_to_num(img_np, nan=0.0, posinf=1.0, neginf=0.0)
    
    # 对于整数类型，确保值在有效范围内并进行四舍五入
    if np.issubdtype(out_type, np.integer):
        img_np = np.clip(img_np, np.iinfo(out_type).min, np.iinfo(out_type).max)
        # 对于整数类型，先四舍五入再转换
...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        ...
        img_np = safe_astype(img_np, out_type)

...
class VMM(nn.Module):
    def __init__(
        ...
        self.A_logs = self.A_log_init(self.d_state, self.d_inner, copies=self.key_num , merge=True)  # (K=4, D, N)
        self.Ds = self.D_init(self.d_inner, copies=self.key_num , merge=True)  # (K=4, D, N)
        ...
        self.relu = nn.ReLU()
        self.att_ca = nn.Conv2d(self.d_inner, 2 *self.d_inner, kernel_size=1, bias=bias)
    ...
    def forward_core(self, x: torch.Tensor, 
        ...
        x_dbl = self.x_proj(xs.reshape(B, -1, L).transpose(1, 2)).transpose(1, 2)
        dts, Bs, Cs = torch.split(x_dbl.reshape(B, K, -1, L), [self.dt_rank, N, N], dim=2)
        #dts = F.conv1d(dts.reshape(B, -1, L), self.dt_projs_weight.reshape(K * D, -1, 1), groups=K)
        #dts = self.dt_proj(dts.reshape(B, -1, L))
        ...
        out_y = self.selective_scan(
            ...
            delta_bias=self.dt_proj.bias.float().view(-1),
            delta_softplus=True,
        ...
        return out_y.view(B, -1, H, W)
    """
    def forward_core(self, x: torch.Tensor, mair_ids, x_proj_bias: torch.Tensor = None):
        B, C, H, W = x.shape
        L = H * W
        K = 1
        
        #print(f"forward_core - x shape: {x.shape}, L={L}")
        
        xs = x  # (B, d_inner, H, W)
        
        # 重塑为 (B, L, d_inner)
        xs_reshaped = xs.reshape(B, C, L).transpose(1, 2)  # (B, L, d_inner)
        #print(f"xs_reshaped shape: {xs_reshaped.shape}")
        
        # x_proj: (B, L, d_inner) -> (B, L, dt_rank + 2*d_state)
        x_dbl = self.x_proj(xs_reshaped)
        #print(f"x_dbl shape: {x_dbl.shape}")
        
        # 分割得到 dts, Bs, Cs
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        #print(f"dts shape: {dts.shape}, Bs shape: {Bs.shape}, Cs shape: {Cs.shape}")
        
        # dt_proj: (B, L, dt_rank) -> (B, L, d_inner)
        dts_projected = self.dt_proj(dts)  # (B, L, d_inner)
        #print(f"dts_projected shape: {dts_projected.shape}")
        
        # 转置回 (B, d_inner, L) 用于selective_scan
        xs_scan = xs.reshape(B, C, L)  # (B, d_inner, L)
        dts_scan = dts_projected.transpose(1, 2)  # (B, d_inner, L)
        Bs_scan = Bs.transpose(1, 2).unsqueeze(1)  # (B, 1, d_state, L)
        Cs_scan = Cs.transpose(1, 2).unsqueeze(1)  # (B, 1, d_state, L)
        
        #print(f"Scan inputs - xs: {xs_scan.shape}, dts: {dts_scan.shape}, Bs: {Bs_scan.shape}, Cs: {Cs_scan.shape}")
        ...
        out_y = self.selective_scan(
            ...
            z=None,
            delta_bias=self.dt_proj.bias.float().view(-1) if hasattr(self.dt_proj, 'bias') and self.dt_proj.bias is not None else None,
            delta_softplus=True,
            return_last_state=False,
        )  # (B, d_inner, L)
        
        #print(f"out_y shape before reshape: {out_y.shape}")
        out_y = out_y.view(B, -1, H, W)
        #print(f"out_y final shape: {out_y.shape}")
    ...
    def forward(self, x: torch.Tensor, mair_ids, **kwargs):
        ...
        #print('x shape=',x.shape) #torch.Size([1, 144, 128, 128]) b,(dim *c), h, w
        x = self.act(self.conv2d(x))
        #print('x activated shape=',x.shape) #torch.Size([1, 144, 128, 128]) b,(dim *c), h, w

        y_0 = self.forward_core(x, mair_ids)
        assert y_0.dtype == torch.float32
        

        #attn0 = y_0
        attn1 = self.relu(y_0)**2
        ...
        #print('attn0.shape=', attn0.shape)
        attn = y_0*(1+scale)+shift


        y = attn.permute(0, 2, 3, 1).contiguous() #torch.Size([1, 128, 128, 144])
        
        y = self.out_norm(y)
        y = y * F.silu(z)
        #print('y silu =',y.shape) #y silu = torch.Size([1, 128, 128, 144])
        y = self.out_proj(y)
        if self.dropout is not None:
            y = self.dropout()

        #print('output y.shape=', y.shape)#torch.Size([1, 128, 128, 96])
        return y
...
class SparseMaIRUNet(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 ssm_ratio=1.5,
        ...
        super(SparseMaIRUNet, self).__init__()

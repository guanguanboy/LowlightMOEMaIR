...
class RMB(nn.Module):
    ...
    def forward(self, input, mair_ids, x_size):
        ...
        if self.shift_size > 0:
            x = self.drop_path(self.self_attention(x, (mair_ids[2], mair_ids[3]))) + self.frequency(x)
        else:
            x = self.drop_path(self.self_attention(x, (mair_ids[0], mair_ids[1]))) + self.frequency(x)
...
class MaIRUNetSFMBStyle(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 ssm_ratio=1.5,
        ...
        super(MaIRUNetSFMBStyle, self).__init__()

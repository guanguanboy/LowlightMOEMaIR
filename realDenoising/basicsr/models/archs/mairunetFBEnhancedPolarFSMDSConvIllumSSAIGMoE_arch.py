...
class IllumAwareShuffleAttn(nn.Module):
    def __init__(self, in_features, out_features, group=4, input_resolution=(64,64)):
        ...
        # --- 核心改进：照度感知门控 ---
        # 我们移除原有的 AdaptiveAvgPool2d(1)，因为照度增强需要空间上的细粒度控制
        self.illum_encoder = nn.Sequential(
            nn.Conv2d(in_features, in_features // 4, kernel_size=3, padding=1, groups=self.group),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_features // 4, out_features, kernel_size=1, groups=self.group),
        )
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        # 2. 通道洗牌：打乱 4 个扫描方向的特征，促进方向间的交互
        x = self.channel_shuffle(identity)
        
        # 3. 照度特征提取：
        # 计算每个位置的平均强度，作为局部的照度先验提示
        # 我们不降维到 1x1，而是保留 HxW 的空间分辨率
        illum_map = torch.mean(x, dim=1, keepdim=True) 
        
        # 4. 生成照度感知的门控权重
        # 将 illum_map 广播回原通道数（或直接通过 conv 处理 x + illum_map）
        # 这里使用 x 配合局部上下文生成权重
        gate_weights = self.illum_encoder(x + illum_map)
        gate_weights = self.sig(gate_weights)
...
class SNRAwareShuffleAttn(nn.Module):
    def __init__(self, in_features, out_features, group=4, input_resolution=(64, 64)):
        super().__init__()
        self.group = group
        self.in_features = in_features
        ...
        # --- 核心改进：SNR 感知编码器 ---
        # 移除全局池化，改为卷积层以保持空间分辨率 (H, W)
        # 这样可以针对图像中不同 SNR 的区域应用不同的聚合权重
        ...
        # 可选：SNR 代理提取器（利用照度估计 SNR）
        self.snr_proxy = nn.Sequential(
            nn.Conv2d(1, 8, kernel_size=3, padding=1),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, x):
        ...
        # 2. 通道洗牌：打乱 K 个扫描方向的特征
        x = self.channel_shuffle(x)
        
        # 3. 提取 SNR 代理图 (SNR Map)
        # 在图像处理中，低亮度区域通常对应低 SNR (噪声大)，高亮度区域对应高 SNR
        illum_map = torch.mean(x, dim=1, keepdim=True)
        snr_map = self.snr_proxy(illum_map) # [B, 1, H, W]
        
        # 4. 生成 SNR 感知的门控权重
        # 将 snr_map 与特征结合，计算空间自适应的注意力
        gate_weights = self.snr_encoder(x * snr_map) 
        gate_weights = self.sig(gate_weights)
...
class VMM(nn.Module):
    def __init__(
        ...
        self.gating = IllumAwareShuffleAttn(in_features=self.d_inner*4, out_features=self.d_inner*4, group=self.d_inner)
...
class MaIRUNetFBEnhancedPolarFSMDSConvIllumSSAIGMoE(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 ssm_ratio=1.5,
        ...
        super(MaIRUNetFBEnhancedPolarFSMDSConvIllumSSAIGMoE, self).__init__()
...
if __name__ == '__main__':
    height = 64
    width = 64
    model = MaIRUNetFBEnhancedPolarFSMDSConvIllumSSAIGMoE(
        inp_channels=3,
        out_channels=3,
        dim=48,
        num_blocks=[4, 6, 6, 8],

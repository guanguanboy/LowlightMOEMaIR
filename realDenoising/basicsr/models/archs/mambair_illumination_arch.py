...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2.):
        ...
        # ✅ 路由网络：接收 dim + 1（附加光照强度）
        self.route = nn.Sequential(
            nn.Linear(self.dim + 1, self.dim // 3),
            nn.GELU(),
    ...
    def forward(self, x, x_size, token):
        ...
        # ✅ Step 1: 计算每个像素的光照强度（L2范数）
        illum = x.norm(p=2, dim=-1, keepdim=True)  # [B, n, 1]

        # ✅ Step 2: 拼接到原始语义特征上
        x_with_illum = torch.cat([x, illum], dim=-1)  # [B, n, C+1]

        # 获取语义字典 token embedding
        full_embedding = self.embeddingB.weight @ token.weight  # [num_tokens, dim]

        # 路由概率分配
        pred_route = self.route(x_with_illum)  # [B, n, num_tokens]
        ...
        # 分组索引生成
        detached_index = torch.argmax(cls_policy.detach(), dim=-1).view(B, n)
        ...
        x_img = x_img * torch.sigmoid(self.CPE(x_img))
        cc = x_img.shape[1]
        x_proj = x_img.view(B, cc, -1).permute(0, 2, 1).contiguous()
        ...
        # 邻居逆重组
        x_out = semantic_neighbor(y, x_sort_indices_reverse)
...
class MambaIRv2_Illumination(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 in_chans=3,
    ...
    def forward(self, x):
        ...
        # unpadding
        x = x[..., :h_ori, :w_ori]
        #print('x.shape=',x.shape, h_ori, w_ori)
        return x

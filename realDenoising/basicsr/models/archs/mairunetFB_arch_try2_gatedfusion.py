...
class RMB(nn.Module):
    def __init__(
        ...
        self.fusion_gate = nn.Linear(hidden_dim * 2, hidden_dim)  # 新增门控
    ...
    def forward(self, input, mair_ids, x_size):
        ...
        ssm_attn = self.self_attention(x, (mair_ids[0], mair_ids[1]))
        freq = self.frequency(x)

        gate = torch.sigmoid(self.fusion_gate(torch.cat([ssm_attn, freq], dim=-1)))
        fused = gate * ssm_attn + (1 - gate) * freq
        fused = self.ln_fusion(fused)

        droped = self.drop_path(fused)

        x = input + self.conv_blk(self.ln_2(droped))

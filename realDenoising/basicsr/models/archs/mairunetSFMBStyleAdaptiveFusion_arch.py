...
class RMB(nn.Module):
    def __init__(
        ...
        self.fuse_module = AdaptiveGateFusion(hidden_dim)
    ...
    def forward(self, input, mair_ids, x_size):
        ...
        if self.shift_size > 0:
            x_ssm = self.drop_path(self.self_attention(x, (mair_ids[2], mair_ids[3])))
        else:
...
class MaIRUNetSFMBStyleAdaptiveFusion(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 ssm_ratio=1.5,
        ...
        super(MaIRUNetSFMBStyleAdaptiveFusion, self).__init__()

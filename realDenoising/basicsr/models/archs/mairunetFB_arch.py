...
class RMB(nn.Module):
    ...
    def forward(self, input, mair_ids, x_size):
        ...
        if self.shift_size > 0:
            x = input*self.skip_scale + self.drop_path(self.self_attention(x, (mair_ids[2], mair_ids[3])) + self.frequency(self.ln_3(input)))
        else:
            x = input*self.skip_scale + self.drop_path(self.self_attention(x, (mair_ids[0], mair_ids[1])) + self.frequency(self.ln_3(input)))

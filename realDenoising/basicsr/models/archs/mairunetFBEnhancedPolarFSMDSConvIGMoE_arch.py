...
class SaSSM_LoSh2D(nn.Module):
    ...
    def forward_core_backup(self, x: torch.Tensor, losh_ids, structure_mod, x_proj_bias: torch.Tensor=None):
        B, C, H, W = x.shape
        L = H * W
        K = 4
        ...
        # --- [改进] 结构感知 NSS：利用结构图调制 x_proj 的结果 ---
        # structure_mod 来源于相位信息，用于调节步长 Delta 和选择性
        ...
        # 核心创新：结构调制。structure_mod 在边缘处权重小，使得 dt 更小，扫描更精细
        x_dbl = x_dbl * structure_mod.view(B, 1, 1, L) 
        ...
        # 传统的 Selective Scan 执行
        xs = xs.float().view(B, -1, L)
        dts = dts.contiguous().float().view(B, -1, L)
        # ... [此处执行 selective_scan_fn，与原代码一致] ...
        ...
        out_y = self.selective_scan(xs, dts, As, Bs, Cs, Ds, delta_bias=dt_projs_bias, delta_softplus=True).view(B, K, -1, L)

        # 返回四个方向的特征，用于后续 SSA 聚合
        return mair_ids_inverse(out_y, xs_inverse_ids, shape=(B, K, C, H, W))
...
class IG_ShuffleAttn(nn.Module):
    def __init__(self, in_features, out_features, group):
        super().__init__()
        self.group = group
        self.in_features = in_features
        
        # 用于处理光照图的轻量级卷积，提取引导权重
        self.illu_conv = nn.Sequential(
            nn.Conv2d(1, 4, kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv2d(4, in_features, kernel_size=1),
            nn.Sigmoid()
        )
        
        # 原有的 Shuffle 操作或额外的空间/通道注意力
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(in_features, in_features, kernel_size=3, padding=1, groups=group),
            nn.Sigmoid()
        )

    def forward(self, x, illu_map):
        """
        x: [B, K*C, H, W] (K=4 directions)
        illu_map: [B, 1, H, W]
        """
        # 1. 从光照图中提取引导权重
        gate_weight = self.illu_conv(illu_map) # [B, K*C, H, W]
        
        # 2. 结合光照引导和空间门控
        # 较暗的区域通过 illu_map 获得不同的注意力增益
        x = x * gate_weight
...
class MaIRUNetFBEnhancedPolarFSMDSConvIGMoE(nn.Module):
    def __init__(self,
                 img_size=64,
                 patch_size=1,
                 ssm_ratio=1.5,
        ...
        super(MaIRUNetFBEnhancedPolarFSMDSConvIGMoE, self).__init__()
...
if __name__ == '__main__':
    height = 64
    width = 64
    model = MaIRUNetFBEnhancedPolarFSMDSConvIGMoE(
        inp_channels=3,
        out_channels=3,
        dim=48,
        num_blocks=[4, 6, 6, 8],

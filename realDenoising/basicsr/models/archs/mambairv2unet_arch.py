...
class AttentiveVSSBlock(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
        ...
        self.self_attention = ASSM(dim=hidden_dim, d_state=d_state, **kwargs)
    ...
    def forward(self, input, x_size):
        ...
        input = input.view(B, *x_size, C).contiguous()  # [B,H,W,C]
        x = self.ln_1(input)
        x = input*self.skip_scale + self.drop_path(self.self_attention(x,x_size, self.embeddingA))
        x = x*self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        x = x.view(B, -1, C).contiguous()
        return x
...
class MambaIRv2UNet(nn.Module):
    def __init__(self,
        ...
        super(MambaIRv2UNet, self).__init__()

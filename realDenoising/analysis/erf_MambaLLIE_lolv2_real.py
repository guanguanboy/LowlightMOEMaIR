...
print(f"当前 Python 进程的工作目录是: {cwd}")

import sys
import os

# 获取当前脚本所在目录的父目录（即包含 basicsr 的那个文件夹）
...
#from basicsr.models.archs.SFIMLLIE_arch import buildSFIMLLIE

from model_zoo.MambaLLIE import buildMambaLLIE
#from model_zoo.SFIMLLIE import buildSFIMLLIE

#/data/lgl/codes/MambaIR/realDenoising/analysis/model_zoo
...
import seaborn
from analysis.model_zoo.swinIR import buildSwinIR
from analysis.model_zoo.rcan import buildRCAN
from analysis.model_zoo.edsr import buildEDSR
...
confi_file_root_path = '../options/test/test_MambaLLIE_LOLv2_real.yml'
opt = parse_options(confi_file_root_path)
opt=opt['datasets']['test'] # we use the 4-th SR testsets(i.e. Urban100) to visualize ERF.
...
def analyze_erf(source, dest="heatmap_MambaLLIE_lolv2real.png", ALGRITHOM=lambda x: np.power(x - 1, 0.25)):
...
# copied from https://github.com/DingXiaoH/RepLKNet-pytorch
def visualize_erf(MODEL: nn.Module = None, num_images=15, data_path="/data/lgl/datasets/LowLight/LOL-v2",
    ...
    def main(args, MODEL: nn.Module = None):
        ...
        for idx,data_sample in enumerate(test_loader):
            ...
            samples = F.interpolate(data_sample['lq'],size=(128,128))
...
if __name__ == '__main__':
    ...
    init_model = buildMambaLLIE()
    ckpt_path = '/data/lgl/codes/LowlightMOEMaIR/pretrained/MambaLLIE_weights/LOLv2_real.pth' # path to load your pre_trained model weights
    init_model.load_state_dict(torch.load(ckpt_path)['params'])
    save_path = f"./tmp/{time.time()}/erf.npy"
    grad_map = visualize_erf(init_model, num_images=10, save_path=save_path)
    analyze_erf(source=grad_map, dest=f"{showpath}/erf_MambaLLIE_lolv2_real_10.png")

...
confi_file_root_path = '../options/test/Retinexformer/test_RetinexFormer_LOL_v1.yml'
...
if __name__ == '__main__':
    ...
    init_model = buildRetinexformer()
    ckpt_path = '/data/lgl/codes/LowlightMOEMaIR/pretrained/Retinexformer_pretrain_model/LOL_v1.pth' # path to load your pre_trained model weights
    init_model.load_state_dict(torch.load(ckpt_path)['params'])
    save_path = f"./tmp/{time.time()}/erf.npy"
    grad_map = visualize_erf(init_model, num_images=15, save_path=save_path)
    analyze_erf(source=grad_map, dest=f"{showpath}/erf_Retinexformer_lolv1_15.png")

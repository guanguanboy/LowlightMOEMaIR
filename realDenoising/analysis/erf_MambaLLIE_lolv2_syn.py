...
confi_file_root_path = '../options/test/test_MambaLLIE_LOLv2_syn.yml'
...
if __name__ == '__main__':
    ...
    ckpt_path = '/data/lgl/codes/LowlightMOEMaIR/pretrained/MambaLLIE_weights/LOLv2_syn.pth' # path to load your pre_trained model weights
    init_model.load_state_dict(torch.load(ckpt_path)['params'])
    save_path = f"./tmp/{time.time()}/erf.npy"
    grad_map = visualize_erf(init_model, num_images=5, save_path=save_path)
    analyze_erf(source=grad_map, dest=f"{showpath}/erf_MambaLLIE_lolv2_syn_5.png")
